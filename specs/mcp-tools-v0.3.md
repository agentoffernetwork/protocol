...
> **Canonical v0.3 tool contract**

This list aligns MCP names with the v0.3 Query and control envelopes. A
deployment claims availability only after its owner provides the required
conformance evidence.

| Tool | Purpose | Contract source | Status |
| --- | --- | --- | --- |
| `aon_search_offers` | Match Offers to the user's current structured intent and return guidance | Query v0.3 + Offer v0.3 | Contract |
| `aon_resolve_category` | Resolve a category reference before querying; returns category metadata, not decision factors | Category resolver contract | Contract |
| `aon_submit_feedback` | Submit an explicit user feedback action for an Offer | Feedback/watches envelope | Contract; deployment-owned |
| `aon_manage_watch` | Explicitly restore or cancel the default Offer-change watch | Feedback/watches envelope | Contract; deployment-owned |
...
- `decision_factors` and generic `next_actions` are not v0.3 MCP fields.
